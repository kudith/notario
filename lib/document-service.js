  ...
  try {
    ...
    if (originalType === 'string') {
      ...
      if (isBase64(signature)) {
        ...
        // For ECDSA, we want to keep base64 format without conversion
        if (algorithm === "ECDSA") {
          return signature;
        }
...
export async function signDocument(file, fileHash, signature, publicKey, options = {}) {
  try {
    ...
    if (options.qrPosition) {
      ...
      // Default to auto-detect signature area
      formData.append('qrPosition', 'signature-area');
    }
    
    // Enable/disable auto-detection
    if (options.autoDetectSignature !== undefined) {
      formData.append('autoDetectSignature', options.autoDetectSignature.toString());
    } else {
      formData.append('autoDetectSignature', 'true'); // Default to enabled
    }
  ...
  try {
    ...
    if (result.verified) {
      toast.success("Document verified", {
        description: `Signed by ${result.documentInfo.signer?.name || 'a verified user'}`
      });
    } else {
      toast.info("Document not verified", {
        description: result.message || "No valid signature found for this document."
      });
    }
...
/**
 * Verify document by certificate ID or hash (LESS SECURE - for convenience only)
...
export async function verifyDocumentByIdentifier(certificateId = null, fileHash = null) {
  try {
    ...
    // Ensure AI analysis data is properly mapped if it exists
    if (result.documentInfo?.metadata?.analysis && !result.documentInfo?.metadata?.aiAnalysis) {
      result.documentInfo.metadata.aiAnalysis = result.documentInfo.metadata.analysis;
    }
    
    // Show success or not found toast based on result
    if (result.verified) {
      toast.success("Document found in records", {
        description: `Signed by ${result.signedBy || 'a verified user'}`
      });
      
      // Show security notice if present
      if (result.securityNotice) {
        toast.info("Security Notice", {
          description: result.securityNotice,
          duration: 6000
        });
      }
    } else {
